pipeline {
  ...
  stages {
    ...
    stage('Deploy to Kubernetes') {
      steps {
        bat """
        powershell -Command "(Get-Content deployment.yaml).replace('bhoomika714442/cloud-native-node-api:latest', '%DOCKER_IMAGE%:%IMAGE_TAG%') | Set-Content k8s-deploy-%IMAGE_TAG%.yaml"
